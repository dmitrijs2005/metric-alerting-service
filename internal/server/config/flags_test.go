...
func TestParseFlags(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Test1 iP:port", args: []string{"cmd", "-a=127.0.0.1:9090", "-i", "30", "-f", "/tmp/tmp.sav", "-d", "db",
			"-k", "secretkey1", "-crypto-key", "some_file.pem", "-t", "192.168.1.0/24", "-r", "true"},
			expected: &Config{EndpointAddr: "127.0.0.1:9090", StoreInterval: 30 * time.Second,
				FileStoragePath: "/tmp/tmp.sav", Restore: true, DatabaseDSN: "db", Key: "secretkey1", CryptoKey: "some_file.pem", TrustedSubnet: "192.168.1.0/24"}}, // Edge case: empty value
